...
public class LookupService
{
    ...
    public synchronized Region getRegion(long ipnum)
    {
        ...
        if (databaseType == DatabaseInfo.REGION_EDITION_REV0) {
            ...
            if (seek_region < US_OFFSET) {
                ...
                record.countryCode =
                        countryCode[(seek_region - WORLD_OFFSET) / FIPS_RANGE];
                record.countryName =
                        countryName[(seek_region - WORLD_OFFSET) / FIPS_RANGE];
                record.region = "";
            }
        }
        return record;
    ...
    public synchronized Location getLocation(long ipnum)
    {
        ...
        try {
            ...
            for (j = 0; j < 3; j++)
                latitude +=
                        (unsignedByteToInt(record_buf[record_buf_offset + j]) << (j * 8));
            ...
            if (databaseType == DatabaseInfo.CITY_EDITION_REV1) {
                ...
                if (record.countryCode == "US") {
                    ...
                    for (j = 0; j < 3; j++)
                        metroarea_combo +=
                                (unsignedByteToInt(record_buf[record_buf_offset
                                        + j]) << (j * 8));
                    record.metro_code =
    ...
    public synchronized String getOrg(long ipnum)
    {
        ...
        try {
            ...
            record_pointer =
                    seek_org + (2 * recordLength - 1) * databaseSegments[0];
            if ((dboptions & GEOIP_MEMORY_CACHE) == 1) {
                // read from memory
