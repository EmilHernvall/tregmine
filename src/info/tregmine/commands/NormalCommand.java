    ...
    public boolean handlePlayer(TregminePlayer player, String[] args)
    {
        if (player.isAdmin()) {
            ...
            if (maxRank != null) {
                ...
                player.sendMessage(BLUE + "You are now in normal mode, and no longer have to response to help requests.");
                maxRank.setGuardianState(TregminePlayer.GuardianState.ACTIVE);
                maxRank.sendMessage(BLUE + "You are now on active duty and should respond to help requests.");
            } else {
                player.sendMessage(BLUE + "Not enough guardians are on to manage the server. We need you to keep working. Sorry. :/");
            }
        }
