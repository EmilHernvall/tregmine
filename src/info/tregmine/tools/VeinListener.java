...
public class VeinListener implements Listener
{
    ...
    public void VeinMiner(BlockBreakEvent event){
        ...
        if(!block.getType().equals(Material.GOLD_ORE) &&
                ...
                !block.getType().equals(Material.LAPIS_ORE)) return;
        ...
        if (Integer.parseInt(durability[0]) < connectedBlocks.size()) { // Leave the player with 1 durability point
            player.sendMessage(ChatColor.RED + "You do not have enough durability to vein miner this ore vein");
            return;
        }
        
        lore.remove(1);
        lore.add(Integer.parseInt(durability[0]) - size + "/1000");
        ...
        player.sendMessage("You vein mined an ore vein containing " + size + " ores!");
